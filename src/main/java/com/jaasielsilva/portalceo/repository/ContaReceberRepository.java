...
public interface ContaReceberRepository extends JpaRepository<ContaReceber, Long> {
    ...
    List<ContaReceber> findByCategoriaAndStatusOrderByDataVencimento(ContaReceber.CategoriaContaReceber categoria, ContaReceber.StatusContaReceber status);
    ...
    @Query("SELECT SUM(c.valorOriginal) FROM ContaReceber c WHERE c.status = :status")
    BigDecimal sumValorOriginalByStatus(@Param("status") ContaReceber.StatusContaReceber status);

    @Query("SELECT SUM(c.valorOriginal + c.valorJuros + c.valorMulta - c.valorDesconto) FROM ContaReceber c WHERE c.status = :status")
    BigDecimal sumValorTotalByStatus(@Param("status") ContaReceber.StatusContaReceber status);
    ...
    @Query("SELECT SUM(c.valorOriginal + c.valorJuros + c.valorMulta - c.valorDesconto - c.valorRecebido) FROM ContaReceber c WHERE c.status IN ('PENDENTE', 'PARCIAL', 'VENCIDA', 'INADIMPLENTE')")
    ...
    @Query("SELECT COUNT(c) FROM ContaReceber c WHERE c.dataVencimento <= :data AND c.status IN ('PENDENTE', 'PARCIAL')")
    Long countVencidas(@Param("data") LocalDate data);

    @Query("SELECT COUNT(c) FROM ContaReceber c WHERE c.status = 'INADIMPLENTE'")
    Long countInadimplentes();
    ...
    @Query("SELECT c.cliente, SUM(c.valorOriginal + c.valorJuros + c.valorMulta - c.valorDesconto - c.valorRecebido) FROM ContaReceber c WHERE c.status IN ('PENDENTE', 'PARCIAL', 'VENCIDA', 'INADIMPLENTE') GROUP BY c.cliente ORDER BY SUM(c.valorOriginal + c.valorJuros + c.valorMulta - c.valorDesconto - c.valorRecebido) DESC")
    List<Object[]> sumSaldoReceberByCliente();

    @Query("SELECT DATE(c.dataRecebimento), SUM(c.valorRecebido) FROM ContaReceber c WHERE c.dataRecebimento BETWEEN :inicio AND :fim GROUP BY DATE(c.dataRecebimento) ORDER BY DATE(c.dataRecebimento)")
    ...
    List<ContaReceber> findByDescricaoContainingIgnoreCase(String descricao);
    ...
    // Relatórios de aging
    @Query("SELECT COUNT(c), SUM(c.valorOriginal + c.valorJuros + c.valorMulta - c.valorDesconto - c.valorRecebido) FROM ContaReceber c WHERE c.dataVencimento BETWEEN :inicio AND :fim AND c.status IN ('PENDENTE', 'PARCIAL', 'VENCIDA', 'INADIMPLENTE')")
