...
public class MapaPermissaoService {
    ...
    public void sincronizarPermissoes() {
        ...
        // Além disso, vamos cadastrar explicitamente algumas importantes para garantir
    ...
    private void preencherMetadadosAutomaticos(MapaPermissao mapa, String permissaoNome) {
        ...
        if (p.contains("_RH_") || p.contains("RH"))
            mapa.setModulo("RH");
        else if (p.contains("_FINANCEIRO_") || p.contains("FINANCEIRO"))
            mapa.setModulo("Financeiro");
        else if (p.contains("_TI_") || p.contains("TI") || p.contains("SUPORTE"))
            mapa.setModulo("TI");
        else if (p.contains("_JURIDICO_") || p.contains("JURIDICO"))
            mapa.setModulo("Jurídico");
        else if (p.contains("_CLIENTES_") || p.contains("CLIENTES"))
            mapa.setModulo("Comercial");
        else if (p.contains("_VENDAS_") || p.contains("VENDAS"))
            mapa.setModulo("Vendas");
        else if (p.contains("_MARKETING_") || p.contains("MARKETING"))
            mapa.setModulo("Marketing");
        else if (p.contains("_ESTOQUE_") || p.contains("ESTOQUE"))
            mapa.setModulo("Estoque");
        else if (p.contains("_PROJETOS_") || p.contains("PROJETOS"))
            mapa.setModulo("Projetos");
        else if (p.contains("DASHBOARD"))
            mapa.setModulo("Dashboard");
        else if (p.contains("ADMIN"))
            mapa.setModulo("Administração");
        else
            mapa.setModulo("Geral");
    ...
    private void cadastrarMapeamentosExplicitos() {
        // Mapeamento explícito para garantir qualidade nos itens principais solicitados
        // pelo usuário

        // Suporte
        salvarMapeamento("TI", "Painel de Suporte", TipoRecurso.MENU, "MENU_TI_SUPORTE",
                "Acesso ao menu principal de suporte");
        salvarMapeamento("TI", "Novo Chamado", TipoRecurso.BOTAO, "CHAMADO_CRIAR", "Botão para criar novos chamados");
        salvarMapeamento("TI", "Atender Chamado", TipoRecurso.ACAO, "TECNICO_ATENDER_CHAMADOS",
                "Permite assumir e atender chamados");
        salvarMapeamento("TI", "Resolver Chamado", TipoRecurso.ACAO, "CHAMADO_RESOLVER",
                "Permite marcar chamado como resolvido");
        ...
        salvarMapeamento("TI", "Exportar Chamados", TipoRecurso.RELATORIO, "CHAMADO_VISUALIZAR",
                "Permite exportar lista de chamados");

        // Dashboard
        salvarMapeamento("Dashboard", "Gráfico de Adesão", TipoRecurso.GRAFICO, "ROLE_DASHBOARD_ADESAO_VISUALIZAR",
                "Visualizar gráfico de adesão");
        salvarMapeamento("Dashboard", "Dashboard Executivo", TipoRecurso.TELA, "DASHBOARD_EXECUTIVO_VISUALIZAR",
                "Acesso à dashboard executiva e visão geral");
        salvarMapeamento("Dashboard", "Card Financeiro", TipoRecurso.GRAFICO, "DASHBOARD_FINANCEIRO_VISUALIZAR",
                "Visualizar indicadores financeiros no dashboard");
        salvarMapeamento("Dashboard", "Card Operacional", TipoRecurso.GRAFICO, "DASHBOARD_OPERACIONAL_VISUALIZAR",
                "Visualizar métricas operacionais no dashboard");
        salvarMapeamento("Dashboard", "Card RH", TipoRecurso.GRAFICO, "ROLE_RH_READ",
                "Visualizar métricas de RH no dashboard");
        salvarMapeamento("Dashboard", "Card Jurídico", TipoRecurso.GRAFICO, "MENU_JURIDICO_DASHBOARD",
                "Visualizar métricas jurídicas no dashboard");

        // RH
        salvarMapeamento("RH", "Menu RH", TipoRecurso.MENU, "MENU_RH", "Acesso ao módulo de Recursos Humanos");
        salvarMapeamento("RH", "Visualizar Colaboradores", TipoRecurso.TELA, "ROLE_RH_READ",
                "Visualizar lista de colaboradores");

        // Operacional / Fornecedores
        salvarMapeamento("Compras", "Cadastrar Fornecedor", TipoRecurso.TELA, "MENU_FORNECEDOR_CADASTRAR",
                "Tela para cadastro de novos fornecedores. Acesse: /fornecedores/novo");
        salvarMapeamento("Compras", "Listar Fornecedores", TipoRecurso.TELA, "MENU_FORNECEDOR_LISTAR",
                "Tela de listagem de fornecedores. Acesse: /fornecedores");
        salvarMapeamento("Compras", "Contratos de Fornecedores", TipoRecurso.TELA, "MENU_FORNECEDOR_CONTRATOS",
                "Gerenciamento de contratos com fornecedores. Acesse: /fornecedores/contratos");
    }
